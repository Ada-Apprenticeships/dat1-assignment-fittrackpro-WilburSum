...
VALUES (
    ...
    1, 
    DATETIME('now')
); -- Values for member_id, location_id, and current date-time for check_in_time
...
FROM 
    attendance -- From the attendance table
WHERE 
    member_id = 7; -- Where the member_id is 7
...
SELECT
    ...
    END AS 
        day_of_week, -- Alias the result as day_of_week
    COUNT(*) AS visit_count -- Count the number of visits
FROM 
    attendance -- From the attendance table
GROUP BY 
    strftime('%w', check_in_time) -- Group by the day of the week
ORDER BY 
    visit_count DESC -- Order by visit_count in descending order
LIMIT 
...
FROM 
    locations l -- From the locations table
JOIN 
    attendance a ON l.location_id = a.location_id -- Join with the attendance table on location_id
GROUP BY 
    l.name; -- Group by location name
