...
CREATE TABLE locations (
    ...
    name VARCHAR(255) CHECK (length(name) <= 255) NOT NULL, 
    address VARCHAR(255) CHECK (length(address) <= 255) NOT NULL,
    phone_number VARCHAR(15) CHECK (length(phone_number) <= 15) NOT NULL,
    email VARCHAR(255) CHECK (length(email) <= 255) NOT NULL,
    opening_hours VARCHAR(255) CHECK (length(opening_hours) <= 255) NOT NULL
);

-- Members Table
CREATE TABLE members (
    member_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    first_name VARCHAR(255) CHECK (length(first_name) <= 255) NOT NULL,
    last_name VARCHAR(255) CHECK (length(last_name) <= 255) NOT NULL,
    email VARCHAR(255) CHECK (length(email) <= 255) NOT NULL,
    phone_number VARCHAR(15) CHECK (length(phone_number) <= 15) NOT NULL,
    date_of_birth DATE NOT NULL,
    join_date DATE NOT NULL,
    emergency_contact_name VARCHAR(255) CHECK (length(emergency_contact_name) <= 255) NOT NULL,
    emergency_contact_phone VARCHAR(15) CHECK (length(emergency_contact_phone) <= 15) NOT NULL
);
...
CREATE TABLE staff (
    ...
    last_name VARCHAR(255) CHECK (length(last_name) <= 255) NOT NULL,
    email VARCHAR(255) CHECK (length(email) <= 255) NOT NULL,
    phone_number VARCHAR(15) CHECK (length(phone_number) <= 255) NOT NULL,
    position VARCHAR(255) CHECK ((length(position) <= 255) AND (position IN ('Trainer', 'Manager', 'Receptionist', 'Maintenance'))) NOT NULL,
...
CREATE TABLE equipment (
    ...
    type VARCHAR(25) CHECK (type IN ('Cardio', 'Strength')),
...
CREATE TABLE classes (
    ...
    name VARCHAR(255) CHECK (length(name) <= 255) NOT NULL,
    description VARCHAR(255) CHECK (length(description) <= 255) NOT NULL,
...
CREATE TABLE memberships (
    ...
    type VARCHAR(255) CHECK ((length(type) <= 255) AND (type in ('Premium', 'Basic'))) NOT NULL,
    start_date DATE NOT NULL,
    end_date DATE NOT NULL,
    status VARCHAR(255) CHECK ((length(status) <= 255) AND (status IN ('Active', 'Inactive'))) NOT NULL,
...
CREATE TABLE class_attendance (
    class_attendance_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    schedule_id INTEGER NOT NULL,
    member_id INTEGER NOT NULL,
    attendance_status VARCHAR(255) CHECK ((length(attendance_status) <= 255) AND (attendance_status IN ('Attended', 'Registered', 'Unattended'))) NOT NULL,
...
CREATE TABLE payments (
    ...
    member_id INTEGER NOT NULL,
    amount DECIMAL(10, 2) CHECK (0 < amount <= 50) NOT NULL,
    payment_date DATE NOT NULL,
    payment_method VARCHAR(255) CHECK((length(payment_method) <= 255) AND (payment_method IN ('Credit Card', 'Bank Transfer', 'PayPal', 'Cash'))) NOT NULL,
    payment_type VARCHAR(255) CHECK((length(payment_type) <= 255) AND (payment_type IN ('Monthly membership fee', 'Day pass'))) NOT NULL,
    FOREIGN KEY (member_id) REFERENCES members(member_id)
);
...
CREATE TABLE member_health_metrics (
    ...
    weight DECIMAL(5, 2) CHECK(0 < weight < 100) NOT NULL,
    body_fat_percentage DECIMAL(5, 2) CHECK (0 < body_fat_percentage < 100) NOT NULL,
    muscle_mass DECIMAL(5, 2) CHECK (0 < muscle_mass < 100) NOT NULL,
    bmi DECIMAL(5, 2) CHECK (0 < bmi < 100) NOT NULL,
    FOREIGN KEY (member_id) REFERENCES members(member_id)
);
